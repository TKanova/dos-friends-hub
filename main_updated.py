...
from auth import (
    Token, UserLogin, UserRegister, UserResponse, PasswordReset, PasswordResetConfirm,
    verify_password, get_password_hash, create_access_token, create_refresh_token,
    verify_token, get_current_user, validate_password
)

load_dotenv()

app = FastAPI(title="DOS Friend's Hub API", version="1.0.0")
...
async def register(user_data: UserRegister):
    ...
    access_token = create_access_token(
        data={"sub": str(result.inserted_id), "username": user_data.username}
    )
...
@app.get("/")
async def root():
    return {"message": "DOS Friend's Hub API with Authentication"}
